...
go 
Select CompanyName, ContactName  -- filter the two columns
...
Select * From vCustomerContacts; -- select from view

-- Data Request: 0302
-- Request: I want a list of customer companies and their contact people, but only the ones in US and Canada
Select CompanyName, ContactName,Country  -- select specific items
From Northwind.dbo.Customers 
Where Country = 'USA' or Country = 'Canada'  -- choose only the two countries
...
Select * From Northwind.dbo.Products;   -- understand data first
go 
Select * From Northwind.dbo.Categories;  -- understand data first
go 
Select CategoryName, ProductName, '$' + cast(p.UnitPrice as nvarchar(100)) -- casting dollar sign
...
Join Northwind.dbo.Categories as c 
    On p.CategoryID = c.CategoryID 
Where c.CategoryName = 'Seafood'    -- choose only the seafood 
Order By CategoryName, ProductName;
go 
